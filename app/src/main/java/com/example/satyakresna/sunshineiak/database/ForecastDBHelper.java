...
public class ForecastDBHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db){
        final String CREATE_DB_SQL = "CREATE TABLE "+ ForecastContract.ForecastEntry.TABLE_NAME + "("
                ...
                + ForecastContract.ForecastEntry.COLUMN_MAX_TEMP + " DATETIME DEFAULT (DATETIME(CURRENT_TIMESTAMP, 'LOCALTIME')))";
        db.execSQL(CREATE_DB_SQL);
    }
    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion){
        db.execSQL("DROP TABLE ID EXIST "+ ForecastContract.ForecastEntry.TABLE_NAME);
        onCreate(db);
    }
    ...
    public void saveForecast(City city, WeatherItem data){
        ...
        values.put(ForecastContract.ForecastEntry.COLUMN_WEATHER_MAIN, data.getWeather().get(0).getDescription());
    ...
    public DailyForecast getSavedForecast(String city){
        ...
        if(total > 0){
            if(cursor.moveToFirst()){
                do {
                    //create list weather, which is only oone item inside
                    List<Weather> weatherList = new ArrayList<>();
                    //Create the weather object, will be inserted to list above
                    ...
                    //create weather object, which will be used to store list data of forecast
                    WeatherItem item = new WeatherItem();
                    //set weather to weatherItem
                    item.setWeather(weatherList);
                    //set temp to weatherItemp
                    item.setTemp(temp);

                    //geting all data from cursor and set it to weatherItem
    ...
    public boolean isDataAlreadyExist(String city){
        ...
        int total = cursor.getCount();
        Log.d(TAG, "Total data already axist -> "+total);
        cursor.close();
        db.close();
        return total > 0;
    }
